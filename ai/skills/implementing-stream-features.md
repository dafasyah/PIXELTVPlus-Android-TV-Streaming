# Skill: Implementing Stream Features

**When to use:** you're adding or changing anything in PIXELTV's stream-sniffing or native-player path — editing `StreamSniffer`, `MediaStream`, `PlayerActivity`, the WebView intercept, or how a `.m3u8`/`.mpd`/`.mp4` is captured and played.

This pipeline is fragile by nature. Follow these conventions or playback silently breaks (black screen / `403`).
...
1. Add the case to `app/src/test/kotlin/com/streamtv/app/stream/StreamSnifferTest.kt` first (a new container type, a tricky query string, a new ad domain).
...
- These can't be JVM unit-tested → verify on a device. See [verifying-on-android-tv](verifying-on-android-tv.md) and report results honestly (which titles played native, which fell back).

## When a site stops working

Sniffing is site-dependent. If native playback regresses after a site changes its player:

1. Confirm the `.m3u8` is still an HTTP request (not `blob:`/MSE) by watching `shouldInterceptRequest`.
