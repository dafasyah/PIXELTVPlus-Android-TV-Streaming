---
name: verifying-on-android-tv
description: Use to manually verify PIXELTV behavior on an Android TV / emulator after a change — D-pad navigation, native player, stream-sniff, and WebView fallback. The Activities aren't unit-tested, so this is how "it works" gets confirmed honestly.
---

# Verifying PIXELTV on Android TV

The UI/player layers can't be unit-tested here. Before claiming a feature works, run it and observe. Report what you actually saw — if you couldn't test something, say so.

**Announce:** "Using verifying-on-android-tv to confirm this change on a device."
...
**Navigation / focus**
- [ ] Splash → Home. Home shows `▶ Mulai Streaming` and `⚙️ Settings` (no 3-site grid). Both are focusable and visibly highlight.
- [ ] D-pad reaches every card; OK activates the focused one.

**Settings**
- [ ] `⚙️ Settings` opens the form. Endpoint field is pre-filled.
- [ ] Enter an invalid URL (`notaurl`) → Save → rejected with a toast, value unchanged.
- [ ] Enter a valid `https://...` → Save → returns; `Mulai Streaming` now loads the new site.
...
- [ ] Open a title that is DRM/`blob:` (no `.m3u8` sniffable) → no pill → the WebView player still plays (ad-blocked). No crash, no black screen.
- [ ] Force a player error (e.g., a stale/expired stream) → toast + return to WebView, not a frozen black screen.
...
- ExoPlayer `403`/source errors on a stream that should play = a dropped replay header (see `implementing-stream-features`).
